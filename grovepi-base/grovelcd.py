...
if sys.version_info >= (3, 0):
    ...
    def setRGB(r,g,b):
      with I2CMaster() as master:
        master.transaction( writing_bytes(DISPLAY_RGB_ADDR,0,0))
        master.transaction( writing_bytes(DISPLAY_RGB_ADDR,1,0))
        master.transaction( writing_bytes(DISPLAY_RGB_ADDR,0x08,0xaa))
        master.transaction( writing_bytes(DISPLAY_RGB_ADDR,4,r))
        master.transaction( writing_bytes(DISPLAY_RGB_ADDR,3,g))
        master.transaction( writing_bytes(DISPLAY_RGB_ADDR,2,b))

    def textCommand(cmd):
      with I2CMaster() as master:
        master.transaction(writing_bytes(DISPLAY_TEXT_ADDR,0x80,cmd))
      ...
      with I2CMaster() as master:
          ...
          for c in text:
            ...
            if count==16 and row==0:
                ...
                row+=1
            count+=1
            master.transaction(writing_bytes(DISPLAY_TEXT_ADDR,0x40,ord(c)))
